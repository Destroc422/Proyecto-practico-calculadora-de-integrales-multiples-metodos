...
class ProfessionalLaTeXRenderer:
    ...
    def convert_to_academic_format(self, original_expression, result_expression, variable="x") -> str:
        ...
        Args:
            ...
            result_expression: Result expression (SymPy or string)
    ...
    def _clean_latex_for_academic_format(self, latex_str: str) -> str:
        """
        Clean LaTeX string for academic block format display
        
        Args:
            latex_str: LaTeX string
            
        Returns:
            Academic-formatted string suitable for block display
        """
        try:
            # Academic LaTeX conversions (strict mathematical notation)
            conversions = {
                # Powers (academic format - standard superscript like Word)
                r'\*\*': '^',
                
                # Fractions (proper LaTeX format)
                r'\\frac{([^}]+)}{([^}]+)}': r'\\frac{\1}{\2}',
                r'\\dfrac{([^}]+)}{([^}]+)}': r'\\frac{\1}{\2}',
                
                # Roots (proper LaTeX format)
                r'\\sqrt{([^}]+)}': r'\\sqrt{\1}',
                r'\\sqrt\[3\]{([^}]+)}': r'\\sqrt[3]{\1}',
                r'\\sqrt\[n\]{([^}]+)}': r'\\sqrt[n]{\1}',
            ...
            # Handle special cases for academic format - Word-style superscript
            result = re.sub(r'([a-zA-Z])\^([0-9]+)', r'\1^{\2}', result)
            result = re.sub(r'([a-zA-Z])\^({[^}]+})', r'\1^\2', result)
            
            # Handle remaining ** cases (should be converted by the main conversion)
