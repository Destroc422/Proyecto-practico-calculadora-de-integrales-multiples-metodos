...
# Create display manager for backward compatibility
class ProfessionalLaTeXDisplayManager:
    ...
    def display_expression_in_frame(self, expression, frame, title: str = "", 
                                  color: str = None, fontsize: int = 16):
        """Display expression in frame"""
        return self.renderer.display_expression_in_frame(expression, frame, title, color, fontsize)
    
    def display_preview(self, expression, frame):
        """Display preview in frame"""
        return self.renderer.display_preview(expression, frame)
    
    def clear_frame(self, frame):
        """Clear frame"""
        return self.renderer.clear_frame(frame)
    
    def clear_all(self):
        """Clear all displays"""
        return self.renderer.clear_all()
